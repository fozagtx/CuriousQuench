...
graph TB
    subgraph "Frontend (Next.js)"
        ...
        B --> C[SubmitRecord Component]
        B --> D[RewardBanner Component]
        E[WalletConnect] --> B
        F[useRewardSubscription Hook] --> D
    end

    subgraph "Backend API Routes"
        G[/api/submit-record<br/>Game Server]
        H[/api/reward-listener<br/>Reward Server]
    end

    subgraph "Somnia SDK"
        I[clientSDK<br/>Read-only]
        J[serverSDK<br/>Write Access]
    end

    subgraph "Somnia Blockchain"
        K[CuriousQuench Contract<br/>ERC20 Token]
        L[Event Stream]
    end

    C -->|POST| G
    G -->|setAndEmitEvent| I
    I -->|RecordValidated Event| L
    L -->|subscribe| H
    H -->|somniaView<br/>getPlayerData| J
    H -->|sessionWrite<br/>grantReward| J
    J -->|Transaction| K
    K -->|RewardGranted Event| L
    L -->|subscribe| F
    F -->|Display| D

    style K fill:#4299e1,stroke:#2b6cb0,color:#fff
